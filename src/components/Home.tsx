...
const masterContainerVariants: Variants = {
  ...
  visible: {
    opacity: 1,
    transition: { 
      staggerChildren: 0.3, // Orchestrates sequence between the Canvas column and Text column
    },
  },
};

const canvasEntranceVariants: Variants = {
  hidden: { opacity: 0, scale: 0.92, y: 20 },
  visible: { 
    opacity: 1, 
    scale: 1, 
    y: 0,
    transition: { duration: 0.8, ease: [0.2, 0.65, 0.3, 1] } // Ultra-smooth deceleration curve
  },
};
...
const textSequenceVariants: Variants = {
  ...
  visible: {
    opacity: 1,
    transition: { staggerChildren: 0.12 }, // Chains inner items (Heading -> Description -> Button)
  },
};
...
export default function Home() {
  ...
  return (
    ...
    <section id="home" className="min-h-[calc(100vh-6rem)] py-12 md:py-20 flex items-center justify-center w-full overflow-hidden">
      <motion.div 
        variants={masterContainerVariants}
        initial="hidden"
        animate="visible"
        className={`flex flex-col ${layoutClass} items-center justify-between gap-12 w-full`}
      >

        {/* Pattern/Portrait Container - Scales down on mobile */}
        <motion.div 
          variants={canvasEntranceVariants} 
        ...
        >
          ...
          <motion.h1 
            ...
            className="text-4xl sm:text-5xl lg:text-7xl font-bold text-text-main mb-6 tracking-tight whitespace-pre-wrap leading-tight transition-colors duration-500"
          >
            <SmartText
              text={typedHeading}
          ...
          <motion.p 
            variants={textChildVariants} 
            className="text-text-muted text-base md:text-lg leading-relaxed mb-10 max-w-lg transition-colors duration-500"
          >
